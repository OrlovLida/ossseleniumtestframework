...
public class OldTable implements TableInterface {
    ...
    private static class Column {
        ...
        private boolean checkIfLabelExist() {
            return this.column.findElements(By.xpath(".//span | .//*[@class='text-ellipsis']")).size() > 0;
        }
    ...
    private static class PredefinedFilter {
        ...
        private static PredefinedFilter createPredefinedFilter(WebDriver driver, WebDriverWait wait, String filterName) {
            ...
            WebElement predefinedFilter = filters.stream().filter(filter -> filter.getText().equals(filterName)).findFirst().orElseThrow(() -> new RuntimeException("There is no Predefined Filter"));
