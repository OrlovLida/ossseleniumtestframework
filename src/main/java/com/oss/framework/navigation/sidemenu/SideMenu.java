...
public class SideMenu {
    ...
    private static final String ACTION_NAME_PATH_PATTERN = MENU_LEVEL_XPATH + "//div[@data-path='%s']";
    ...
    private static final String SIDE_MENU = "sideMenu.";
    ...
    public void callActionByLabel(String testid, String... paths) {
        ...
        expandSideMenu();
        moveToTopOfSideMenu();
        StringBuilder currentPath = new StringBuilder(SIDE_MENU);
        for (String path : paths) {
            currentPath.append(path);
            if (!isPathExpanded(currentPath.toString())) {
                callAction(currentPath.toString());
                waitForClickedPath(currentPath.toString());
            }
            currentPath.append(".");
        }
        String applicationPath = currentPath.append(testid).toString();
        callAction(applicationPath);
        waitForClickedAction(applicationPath);
    }
    ...
    private WebElement moveToElement(String xpath) {
        WebElement foundElement = getElement(xpath);
    ...
    private void waitForClickedAction(String testid) {
        String actionXpath = String.format(ACTION_NAME_PATH_PATTERN, testid);
        WebElement action = getElement(actionXpath);
    ...
    private void waitForClickedPath(String path) {
        ...
        try {
            ...
            clickOnElement(getElement(pathXpath));
            shortWait.until(ExpectedConditions.attributeContains(By.xpath(pathXpath), CLASS, IS_EXPANDED));
        }
    ...
    private boolean isPathExpanded(String path) {
        ...
        return getElement(pathXpath).findElement(By.className("own-chevron")).getAttribute("aria-label").equals("Collapse");
    }
    ...
    private WebElement getElement(String pathXpath) {
        return getSideMenu().findElement(By.xpath(pathXpath));
    }
}
